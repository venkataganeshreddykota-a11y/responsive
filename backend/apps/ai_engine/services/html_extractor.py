...
def _clean_text(value):
    """Drop invalid surrogate characters before values reach JSON rendering."""
    return str(value or "").encode("utf-8", "ignore").decode("utf-8")


def _normalize_css(css_text):
    css = re.sub(r"</?span[^>]*>", "", _clean_text(css_text), flags=re.IGNORECASE)
...
def extract_from_url(url):
    ...
    for index, script in enumerate(soup.find_all("script"), start=1):
        ...
        if body:
            js_chunks.append({
                ...
                "url": "",
                "js": _clean_text(body)[:12000],
            })

    for src in script_sources[:8]:
        if src.startswith("//"):
            js_url = f"{parsed.scheme}:{src}"
        elif src.startswith("/"):
            js_url = origin + src
        else:
            js_url = urljoin(base_url, src)
        try:
            js_response = requests.get(js_url, headers={**HEADERS, "Accept": "application/javascript,text/javascript,*/*;q=0.1", "Referer": base_url}, timeout=4)
            if js_response.ok:
                js_chunks.append({
                    "source": f"external script: {js_url}",
                    "url": js_url,
                "js": _clean_text(js_response.text)[:12000],
                })
        except requests.RequestException:
            js_chunks.append({
                "source": f"blocked external script: {js_url}",
                "url": js_url,
                "js": "",
            })
    ...
    js_parts = []
    total = 0
    for item in js_chunks:
        header = f"/* Source: {item['source']} */"
        part = f"{header}\n{item['js']}"
        if total + len(part) > JS_LIMIT:
            remaining = JS_LIMIT - total
            if remaining > 1200:
                js_parts.append(part[:remaining])
            break
        js_parts.append(part)
        total += len(part)

    return {
        "html": _clean_text(str(soup))[:60000],
        "css": _clean_text("\n\n".join(css_parts)),
        "js": _clean_text("\n\n".join(js_parts)),
        "css_sources": [
        ...
        "js_sources": [
            {"source": item["source"], "url": item["url"], "size": len(item["js"])}
            for item in js_chunks
        ],
        "dom": {
