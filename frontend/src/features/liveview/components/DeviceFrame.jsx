...
function DeviceFrame({
  ...
  const { canvasRef, reconnect, send, sendRequest, status: streamStatus, error } = useDeviceStream({
    url,
    device,
    reloadToken,
    streamScale,
  ...
  useImperativeHandle(ref, () => ({
    ...
    inspectPage() {
      return sendRequest({ type: "inspect_code" }, 20000);
    },
    applyFix({ css = "", html = "", js = "" } = {}) {
      return sendRequest({ type: "apply_css", css, html, js }, 20000);
    },
    applyCss(css) {
      return sendRequest({ type: "apply_css", css: css || "", html: "", js: "" }, 20000);
    },
    isReady() {
      return streamStatus === "ready";
    },
  }), [sendRequest, streamStatus]);
