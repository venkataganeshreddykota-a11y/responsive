...
import { FiSmartphone, FiTablet, FiClock, FiX } from "react-icons/fi";
        ...
        {!loading && !result ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-4 px-6 text-center">
            <div className="flex gap-5 opacity-20 text-surface-muted">
              <FiSmartphone size={40} /><FiTablet size={40} /><MdLaptop size={40} />
            </div>
            ...
            <div ref={issuesSectionRef} className="scroll-mt-4 px-4 py-5 sm:px-6">
              ...
              {loading && (
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {[1,2,3].map(n => <SkeletonPanel key={n} h="h-64" />)}
                </div>
              )}
            </div>
          </div>
