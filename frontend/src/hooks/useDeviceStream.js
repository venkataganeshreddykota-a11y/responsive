...
export default function useDeviceStream({
  ...
  useEffect(() => {
    ...
    wsRef.current = ws;
      ...
      Object.values(pendingRequestsRef.current).forEach(({ reject }) => {
        reject(new Error("Live browser stream restarted."));
      });
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === "response" && message.requestId) {
        const pending = pendingRequestsRef.current[message.requestId];
        if (!pending) return;
        window.clearTimeout(pending.timer);
        delete pendingRequestsRef.current[message.requestId];
        if (message.ok === false) {
          pending.reject(new Error(message.error || "Live browser request failed."));
        } else {
    ...
    return () => {
      alive = false;
      Object.values(pendingRequestsRef.current).forEach(({ reject }) => {
        reject(new Error("Live browser stream closed."));
      });
      pendingRequestsRef.current = {};
  ...
  const sendRequest = useCallback((payload, timeoutMs = 15000) => new Promise((resolve, reject) => {
    if (wsRef.current?.readyState !== WebSocket.OPEN) {
      reject(new Error("Live browser stream is not connected yet."));
      return;
    }
    const requestId = `req-${Date.now()}-${requestIdRef.current += 1}`;
    const timer = window.setTimeout(() => {
      delete pendingRequestsRef.current[requestId];
      reject(new Error("Live browser request timed out."));
    }, timeoutMs);
    pendingRequestsRef.current[requestId] = { resolve, reject, timer };
  ...
  return { canvasRef, reconnect, send, sendRequest, ...state };
}
