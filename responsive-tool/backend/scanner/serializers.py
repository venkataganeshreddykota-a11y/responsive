...
class ScanReportSerializer(serializers.ModelSerializer):
    # Computed fields derived from raw_result["processed"] — served directly
    # so the frontend never has to dig into raw_result itself.
    ...
    class Meta:
        ...
        fields = (
            "id", "status", "score",
            # raw lists (still useful for custom clients)
            "issues", "suggestions",
            "screenshots", "device_results",
            # processed / display-ready fields
    ...
    def _processed(self, obj):
        """Return cached processed dict, computing on-the-fly if missing."""
        # Cache on the instance to avoid recomputing for each get_* method call
    ...
    def get_verdict(self, obj):
        return self._processed(obj).get("verdict")

    def get_verdict_label(self, obj):
        return self._processed(obj).get("verdict_label")

    def get_verdict_detail(self, obj):
        return self._processed(obj).get("verdict_detail")

    def get_issue_groups(self, obj):
        return self._processed(obj).get("issue_groups", [])

    def get_device_status(self, obj):
        return self._processed(obj).get("device_status", [])

    def get_resolution_advice(self, obj):
        return self._processed(obj).get("resolution_advice", [])
