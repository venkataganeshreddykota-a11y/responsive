...
// ── Flat mode (legacy / device-filtered view) ─────────────────────────────────
function FlatIssueList({ issues }) {
  if (!issues.length) {
    return (
      <div className="issue-empty">
        <span>✅</span>
        <p>No issues detected for this viewport.</p>
      </div>
    );
  }
  return (
        ...
        return (
          <li key={i} className={`issue-item ${issue.highlighted ? "issue-item--highlight" : ""}`}>
            ...
            <div className="issue-body">
              ...
              {issue.device && (
                <span className="issue-device">Affects: {issue.device}</span>
              )}
            </div>
          </li>
        );
...
// ── Grouped mode (full result view) ──────────────────────────────────────────
function GroupedIssueList({ groups }) {
  const [open, setOpen] = useState(() => {
    // auto-open critical groups
    const s = {};
              ...
              <ul className="issue-list issue-list--inset">
                {group.issues.map((issue, ii) => {
                  ...
                  return (
                    <li
                      key={ii}
                      className={`issue-item ${issue.highlighted ? "issue-item--highlight" : ""}`}
                    >
  ...
  return (
    <div className="issue-panel">
      <div className="issue-panel-header">
        <h3>{title}</h3>
        {totalCount > 0 && (
          <span className="badge badge-error">{totalCount} total</span>
        )}
      </div>

      {useGrouped
        ? <GroupedIssueList groups={issueGroups} />
