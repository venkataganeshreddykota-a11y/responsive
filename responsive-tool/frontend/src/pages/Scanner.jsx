...
export default function Scanner() {
  ...
  // Advance the fake progress steps while waiting
    ...
    pollRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (data.status === "completed") {
          ...
          stopAll();
          setError(data.raw_result?.error || "Scan failed on the server.");
          setLoading(false);
        }
  ...
  const handleScan = async (e) => {
    ...
    try {
      ...
      stopAll();
      setError(err.response?.data?.url?.[0] || "Failed to start scan.");
      setLoading(false);
    }
  };
      ...
      <main className="scanner-main">
        ...
        <section className="search-section">
          ...
          <p className="search-subtitle">
            Real Playwright screenshots at 375px · 768px · 1280px · 1440px with
            automated overflow, scaling, and layout issue detection.
          </p>
          ...
          <form className="search-bar" onSubmit={handleScan}>
            ...
            <button type="submit" className="btn-primary btn-lg" disabled={loading}>
              {loading
                ? <><span className="spinner-sm" /> {step.label}</>
          ...
          {loading && (
            <div className="scan-progress" role="progressbar" aria-valuenow={step.pct} aria-valuemax={100}>
              ...
              <div className="scan-progress-steps">
                {SCAN_STEPS.map((s, i) => (
                  <span key={s.key} className={`scan-step ${i <= stepIdx ? "scan-step--done" : ""}`}>
                    {s.label}
                  </span>
                ))}
              </div>
        ...
        {(loading || result) && (
          <div className="results-section">
            ...
            {/* 3. Resolution advice + Issues side-by-side */}
            {result && (
