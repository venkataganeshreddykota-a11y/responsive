    ...
    <div className={`device-card ${focused ? "device-card--focused" : ""}`}>
      ...
      <div className="device-frame" style={{ width: frameW, height: frameH }}>
        {isLoading ? (
          ...
          <img src={`data:image/png;base64,${screenshot}`} alt={`${cfg.label} screenshot`}
            style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
        ) : url ? (
          <iframe src={url} title={`${cfg.label} preview`}
            style={{ width: cfg.width, height: cfg.height, transform: `scale(${scale})`,
                     transformOrigin: "top left", border: "none", pointerEvents: "none" }} />
        ) : (
          <div className="frame-empty"><span>{cfg.icon}</span><p>Preview will appear here</p></div>
        )}
      </div>
